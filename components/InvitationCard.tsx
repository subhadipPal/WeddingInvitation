...
interface Props { lang: Lang; translations: Translations; invitedDays?: '22+23' | '23' | '28'; isMulti?: boolean }

function tv(lang: Lang, isMulti: boolean, singular: string, multi: string) {
  // Berlin uses DE plural forms; Hindu invite uses BN plural forms
...
export default function InvitationCard({ lang, translations, invitedDays, isMulti = false }: Props) {
  const hindu = invitedDays === '28'

  const heading = hindu ? translations.hindu_inviteHeading : translations.inviteHeading
  const closing = hindu ? translations.hindu_inviteClosing : translations.inviteClosing
  const body = hindu
  ...
  return (
    <div className="animate-card-rise bg-[#f5f0e8] text-[#1a0a0a] rounded-2xl p-5 sm:p-8 max-w-lg mx-auto text-center shadow-2xl border border-[#c9a84c]/40">
      <p className="font-script text-3xl sm:text-5xl text-[#4a0a0a] mb-3 sm:mb-4">{heading}</p>
      ...
      <p className="font-script text-xl sm:text-2xl text-[#4a0a0a]">{closing}</p>
    </div>
  )
}
