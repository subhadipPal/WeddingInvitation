...
export async function POST(req: NextRequest) {
  const { token, attending22, attending23, attending28, address, note } = await req.json()
  if (!token || (attending23 === undefined && attending28 === undefined)) {
    return NextResponse.json({ error: 'token and an attendance answer required' }, { status: 400 })
  }
  ...
  if (existing.length) {
    await db.update(rsvps)
      .set({
        ...
        attending23: attending23 ?? null,
        attending28: attending28 ?? null,
